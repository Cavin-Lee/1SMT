...
classnum_source=10; classnum_target1=6; classnum_target2=6; classnum_target3=6
M=3   #the number of target domain

Source=scipy.io.loadmat('CADW/A--DWC(4)/Amazon.mat')
Target1=scipy.io.loadmat('CADW/A--DWC(4)/Dslr(4).mat')      # first target
Target2 =scipy.io.loadmat('CADW/A--DWC(4)/Webcam(4).mat')      # second target
Target3 =scipy.io.loadmat('CADW/A--DWC(4)/Caltech(t).mat')      # third target
...
pca_s=KernelPCA(n_components=80,kernel='rbf')
pca_t1=KernelPCA(n_components=80,kernel='rbf')
pca_t2=KernelPCA(n_components=80,kernel='rbf')
pca_t3=KernelPCA(n_components=80,kernel='rbf')
...
scipy.io.savemat('CADW/ADWC.mat',{'Xs':data_source, 'D':data_target1,
                                  'W': data_target2, 'C':data_target3})
...
for param_beta in range(len(beta)):
    for param_gama in range(len(gama)):
        ...
        NMI_t1_parameter[param_beta, param_gama] = round(NMI_t1, 4)
        RI_t1_parameter[param_beta, param_gama] = round(RI_t1, 4)
        ...
        NMI_t2_parameter[param_beta, param_gama] = round(NMI_t2, 4)
        RI_t2_parameter[param_beta, param_gama] = round(RI_t2, 4)
        ...
        NMI_t3_parameter[param_beta, param_gama] = round(NMI_t3, 4)
        RI_t3_parameter[param_beta, param_gama] = round(RI_t3, 4)

        print(NMI_t1_parameter)
        print(NMI_t2_parameter)
        print(NMI_t3_parameter)
        scipy.io.savemat('CADW/A--DWC(4)/beta_gama3.mat',
