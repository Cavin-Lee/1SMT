...
NMI_29_parameter = np.mat(np.zeros((7,1)))
RI_07_parameter = np.mat(np.zeros((7,1)))
RI_29_parameter = np.mat(np.zeros((7,1)))
...
for param_eta in range(len(eta)):
    ...
    for m in range(Maxiter):
        ...
        W_PIE29 = target_domain.target_WTj(data_PIE29, U_PIE29_tar, W_PIE27, VT_tar[1], V_tar[1],
                                           D_tar, Lambda_PIE29, gama_tar[1], beta)
        ...
        V_tar[1] = target_domain.Target_Vj(W_PIE29, D_tar, gama_tar[1], eta[param_eta])
        J_PIE07 = J_new_PIE07
        J_PIE29 = J_new_PIE29
        '''Dslr域的目标函数第二项的值'''
        J2_PIE07 = 0
    ...
    NMI_29_parameter[param_eta,0] = round(NMI_PIE29, 4)
    RI_29_parameter[param_eta, 0] = round(RI_PIE29, 4)
    print(NMI_07_parameter)
    print(RI_07_parameter)
    print(NMI_29_parameter)
    print(RI_29_parameter)
    scipy.io.savemat('PIE/27/07,29/07+29(NMI,RI)_eta.mat', {'NMI_07': NMI_07_parameter, 'RI_07': RI_07_parameter,
                                                        'NMI_29': NMI_29_parameter, 'RI_29': RI_29_parameter})