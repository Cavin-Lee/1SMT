...
Amazon=scipy.io.loadmat('CADW/A--DW(6)/Amazon.mat')
Dslr=scipy.io.loadmat('CADW/A--DW(6)/Dslr(6).mat')      #第一个目标域
Webcam=scipy.io.loadmat('CADW/A--DW(6)/Webcam(6).mat')      #第二个目标域
...
U_Dslr=np.mat(Dslr['Ut'])
U_Webcam=np.mat(Webcam['Ut'])
'''降维'''
...
scipy.io.savemat('CADW/data_ADW4.mat',
                 {'data_Webcam': data_Webcam, 'U_Amazon': U_Amazon,'label_A': label_A,
                  'data_Amazon': data_Amazon, 'U_Dslr': U_Dslr,'label_D': label_D,
                  'data_Dslr': data_Dslr, 'U_Webcam': U_Webcam,'label_W': label_W})
...
for param_beta in range(len(beta)):
    for param_gama in range(len(gama)):
        ...
        for m in range(Maxiter):
            ...
            VT_tar[0] = target_domain.Source_Target_VTj(W_Dslr, W_Amazon, beta[param_beta], eta)
            VT_tar[1] = target_domain.Source_Target_VTj(W_Webcam, W_Amazon, beta[param_beta], eta)
            WT = [W_Dslr, W_Webcam]
            try:
            ...
            J3 = 0
            for j in range(M):
                J3 = J3 + 0.5 * beta[param_beta] * np.linalg.norm(W_Amazon - WT[j] * VT_tar[j], ord=2) ** 2
        ...
        scipy.io.savemat('CADW/A--DW(6)/beta_gama2.mat',
