...
classnum_PIE27=20; classnum_PIE07=9; classnum_PIE09=9; classnum_PIE29=10
M=3   #目标域个数

PIE27=scipy.io.loadmat('PIE/27/PIE27.mat')
PIE07=scipy.io.loadmat('PIE/27/07,09,29/PIE07.mat')      #第一个目标域
PIE09=scipy.io.loadmat('PIE/27/07,09,29/PIE09(1).mat')      #第二个目标域
PIE29=scipy.io.loadmat('PIE/27/07,09,29/PIE29(1-5,10-14).mat')     #第三个目标域
data_PIE27=PIE27['Xs']; data_PIE07=PIE07['Xt']; data_PIE09=PIE09['Xt']; data_PIE29=PIE29['Xt']
label_27=PIE27['Ys']; label_07=PIE07['Yt']; label_09=PIE09['Yt']; label_29=PIE29['Yt']
...
data_PIE27=pca_27.fit_transform(data_PIE27)  #n*d'
...
'''Preprocessing'''
data_PIE27=np.mat(preprocessing.scale(data_PIE27)).T   #d'*n
...
label_PIE27=[]; label_PIE07=[]; label_PIE09=[]; label_PIE29=[]
...
U1 = U_PIE09; U2=U_PIE09; U3=U_PIE29
...
for param_beta in range(len(beta)):
    for param_gama in range(len(gama)):
        ...
        for m in range(Maxiter):
            ...
            W_PIE29 = target_domain.target_WTj(data_PIE29, U_PIE29_tar, W_PIE27, VT_tar[2], V_tar[2],
            ...
            VT_tar[2] = target_domain.Source_Target_VTj(W_PIE29, W_PIE27, beta[param_beta], eta)
            ...
            if (abs((J_new_PIE07 - J_PIE07) / J_new_PIE07) + abs((J_new_PIE09 - J_PIE09) / J_new_PIE09) + abs(
                ...
                break
                # print(abs((J_new_PIE07 - J_PIE07) / J_new_PIE07),abs((J_new_PIE09 - J_PIE09) / J_new_PIE09),
                #   abs((J_new_PIE29 - J_PIE29) / J_new_PIE29))
        print(m)
        print('PIE07域')
